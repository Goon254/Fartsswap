...
export function renderShareCardHtml(payload: ShareCardRenderPayload): string {
  ...
  const html = [
    ...
    '.score-fill{height:100%;width:' + String(score) + '%;background:' + accent + ';border-radius:4px}',
    ...
    '<section class="body">',
    field('Power Score', String(payload.powerScore) + ' / 100'),
    '<div class="score-bar"><div class="score-fill"></div></div>',
    field('Duration', String(payload.durationMs) + ' ms'),
